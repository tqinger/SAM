...
class VOCSegmentation(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_min = torch.clamp(x_min - random.randint(int(wide) * -0.25, int(wide) * 0.25), min=0)
        x_max = torch.clamp(x_max + random.randint(wide * -0.25, wide * 0.25), max=W - 1)
        y_min = torch.clamp(y_min - random.randint(height * -0.25, height * 0.25), min=0)
        y_max = torch.clamp(y_max + random.randint(height * -0.25, height * 0.25), max=H - 1)
...
parser.add_argument(
    ...
    "--tr_npy_path",
    type=str,
    default=r"C:\Users\Public\data\she\514_yuantu",
