...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, target):
        if random.random() < self.flip_prob:
            ...
            # w, _ = image.size
            # boxes = boxes.clone()
            # boxes[:, [0, 2]] = w - boxes[:, [2, 0]]  # x_min, x_max = w - x_max, w - x_min
        return image, target
...
class CenterCrop(object):
    ...
    def __call__(self, image, target):
        ...
        target = F.crop(target, top, left, crop_height, crop_width)

        return image, target


# 如果图像或目标小于指定尺寸的辅助函数，你需要根据实际情况来实现这个函数
def pad_if_smaller(img, size, fill=0):
    min_size = min(img.size)
    if min_size < size:
        ow, oh = img.size
